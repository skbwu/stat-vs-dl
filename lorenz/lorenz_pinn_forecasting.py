...
# fixed settings
TT, TFC, t_max, n_epochs = 0.0, 0.0, 2.0, 60000
...
# reference points for physics-based loss evaluation (accounting for forecasting)
t_max_pred = 5.0 # i.e., end of forecasting period
t_physics = tf.convert_to_tensor(np.linspace(TT, t_max_pred, 
                                             int(((t_max_pred - TT) * 40) + 1))\
                                 .reshape(-1, 1))

# train the PINN for 60K epochs, with physics-loss including forecasting interval!
for epoch in range(n_epochs):

    # one optimization step - note that `is_forecasting` is vestigial / deprecated!
...
if foldername not in os.listdir("results/pinn_forecasting"):
    os.mkdir(f"results/pinn_forecasting/{foldername}")
...
logs.to_csv(f"results/pinn_forecasting/{foldername}/logs.csv", index=False)

# 2. PINN weights after last epoch.
pinn.NN.save_weights(f"results/pinn_forecasting/{foldername}/pinn.weights.h5")

# 3. final estimate of theta_hat
theta_hat_final = pd.DataFrame(data=theta_hat.reshape(1, -1), columns=["beta", "rho", "sigma"])
theta_hat_final.to_csv(f"results/pinn_forecasting/{foldername}/theta_hats.csv", index=False)

# 4. in-sample predictions of (X, Y, Z)
preds = pd.DataFrame(data=np.hstack([lorenz_data[0], X_hat, Y_hat, Z_hat]), columns=["t", "X", "Y", "Z"])
preds.to_csv(f"results/pinn_forecasting/{foldername}/in-sample_preds.csv", index=False)
...
physics_logs.to_csv(f"results/pinn_forecasting/{foldername}/physics_residuals.csv", index=False)

# 6. noisy observations
noisy_obs = pd.DataFrame(data=np.hstack([ts_obs.reshape(-1, 1), X_obs, Y_obs, Z_obs]), 
                         columns=["t", "X", "Y", "Z"])
noisy_obs.to_csv(f"results/pinn_forecasting/{foldername}/noisy_obs.csv", index=False)

# 7. out-of-sample predictions
oos_preds = pd.DataFrame(data=np.hstack([t_physics, np.hstack(pinn.NN(t_physics))]), 
                         columns=["t", "X", "Y", "Z"])
oos_preds.to_csv(f"results/pinn_forecasting/{foldername}/oos_preds.csv", index=False)